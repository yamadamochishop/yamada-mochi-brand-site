...
| `popularity?.score`                                                 | 人気順ソートの総合スコア      | 未設定                                                 | Human／後述の算出式                      |
| `popularity?.searchConsoleClicks` `…Impressions` `…Ctr` `…Position` | Search Console の転記         | 未設定                                                 | Human（GSC接続後）                       |
| `popularity?.measuredAt` `popularity?.period`                       | 「いつの・どの期間の値か」    | 未設定                                                 | 数値を入れるときに必須（validateで強制） |
| `publishedAt`                                                       | 新着順ソート、`datePublished` | 全13件に設定（PRマージ日: 8/28 ×7、9/11 ×6）           | 設定済み                                 |

検証（`lib/content-model/validate.ts`）: `difficulty` の値域、`tags` の空・重複、`popularity` の負値・CTR範囲（0〜1）・整数・日付形式、数値がある場合の `measuredAt` 必須、`publishedAt`/`updatedAt` の `YYYY-MM-DD`。

### 2.1 人気指標の転記手順（Search Console 接続後）

1. GSC「検索パフォーマンス」→ ページ でフィルタ `/recipes/` → 直近28日をエクスポート
2. 各レシピの `popularity` に `searchConsoleClicks` / `searchConsoleImpressions` / `searchConsoleCtr`（比率）/ `searchConsolePosition` と `measuredAt`（転記日）/ `period` を入れる
3. `score` は当面 **clicks をそのまま使う**（`lib/recipe-page.ts` の `popularityScore` は `score` が無ければ `searchConsoleClicks` を暫定スコアにする）。式を変えるときはこの節を更新する
...
H2 注目レシピ … getFeaturedRecipes(3)。スマホは横スクロール、md以上は3列
H2 すべてのレシピ … RecipeExplorer（検索・お餅で絞り込み・並び順） + カード全件
H2 お餅から探す（既存。商品名リンクは recipe_to_product_click を送る）
CTA（既存）
```

- 「注目レシピ」の並び: `featured` → `popularity` 降順 → データ順。実測が無い今はデータ順の先頭3件（焼き方・磯辺焼き・海老餅チーズピザ）が仮の並びになる。**Human が `featured: true` を付ければ即座に差し替わる。**
...
**画像ポリシーの変更（YM-003B / YM-007 から）**: 写真が無いレシピには文字だけのプレースホルダ枠（「写真は準備中です」）を出す。
理由は今回の依頼（後から画像を追加しやすい状態にする）。画像を生成して埋めることはしない。
...
| `recipe_filter_use`       | `filter_type`(`product`\|`sort`\|`tag`) `value` `result_count` | お餅チップ・並び順の変更                                                                                           |
...
| 人気順ソート       | `popularity` を転記するだけ。UIの「人気順」は既にある（未設定は末尾に落ちる）                              | `sortRecipes('popular')`、`popularityScore` |
| 人気レシピ TOP3    | 「注目レシピ」がそのまま人気順になる。見出しを「人気レシピ」に変えるだけ                                   | `getFeaturedRecipes()`                      |
...
- 「注目レシピ」に出したい3件があれば `featured: true` を付ける（無ければデータ順の仮並びのまま）
- プレースホルダ枠を出す方針（3.1）で良いか。NGなら `RecipeCard` の placeholder 分岐を外すだけで YM-007 の挙動に戻る
